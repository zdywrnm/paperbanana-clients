  ...
  return (
    <div className="reference-library-panel">
      ...
      <div className="reference-library-grid">
        {references.map((item) => {
          // SDK normalizeRetrievedReference (packages/api/src/jobs.js) exposes the field as
          // snake_case `image_url`. Keep camelCase `imageUrl` as a fallback so a raw/un-normalized
          // upstream object would still render.
          ...
          return (
            ...
            >
              {imageUrl ? <img src={imageUrl} alt={item.title} loading="lazy" /> : <span className="reference-card-placeholder">PB</span>}
