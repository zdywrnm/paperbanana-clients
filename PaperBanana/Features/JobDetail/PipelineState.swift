...
struct PipelineState: Equatable {
  ...
  init(job: Job) {
    ...
    for (index, stage) in job.stages.enumerated() {
      guard let id = NodeID(stageType: stage.type) else { continue }
    ...
    let activeID = job.stages.reversed().lazy
      .compactMap { NodeID(stageType: $0.type) }
      .first
